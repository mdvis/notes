...
```
`journalctl` 是 Linux 中的一个强大的工具，用于查看和管理系统日志。以下是它的详细说明：
...
journalctl [选项] [匹配项]
```
* `选项`: 修改 `journalctl` 行为的各种选项
* `匹配项`: 应用于日志输出的 filters 或 searches
...
* `-k`: 显示内核消息
* `-b`: 显示启动日志
* `-p 优先级`: 显示具有指定优先级的日志（例如 `debug`、`info`、`warning` 等）
* `-f`: 实时跟踪日志输出
* `-n 行数`: 显示最后 `行数` 行的日志输出
* `-o 格式`: 指定输出格式（例如 `short`、`pretty`、`json` 等）

**过滤器和搜索**

* `--since 时间`: 显示从指定时间开始的日志
* `--until 时间`: 显示直到指定时间的日志
* `--boot ID`: 显示指定启动 ID 的日志
* `--priority=优先级`: 显示具有指定优先级的日志
* `--fields=字段1,字段2,...`: 显示指定字段在日志输出中