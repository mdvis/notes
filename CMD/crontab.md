...
特殊字符
- `*`：所有可能值（如 `*` 在小时字段表示每小时）。
- `,`：分隔多个值（如 `1,3,5` 表示1点、3点、5点）。
- `-`：定义范围（如 `1-5` 表示1到5点）。
- `/`：指定间隔（如 `*/5` 在分钟字段表示每5分钟）。
 常用示例
1. **每天凌晨2点执行脚本**：
...
8. **环境变量**：  
    cron 默认环境简单，建议在脚本中使用绝对路径或在 crontab 中设置 `PATH`：
`PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin`
1. **时区问题**：  
    cron 使用系统时区，修改需调整系统时区设置（如 `timedatectl set-timezone Asia/Shanghai`）。