...
> ⚠️ **注意**：允许 root 远程登录会增加服务器被暴力破解的风险，**强烈建议仅在必要时临时开启，并使用强密码**。更安全的做法是**禁止 root 直接登录，并通过普通用户登录后切换至 root**。
...
在客户端计算机上使用 `ssh-keygen` 命令生成公钥和私钥对：
...
将生成的公钥（默认为 `~/.ssh/id_rsa.pub`）内容添加到服务器的相应用户的 `~/.ssh/authorized_keys` 文件中。你可以使用 `ssh-copy-id` 命令方便地完成（如果服务器已配置密码认证）：
...
服务器上 `.ssh` 目录的权限应为 `700`，`authorized_keys` 文件的权限应为 `600`：
...
确保服务器的 `/etc/ssh/sshd_config` 中包含：
...
*   **限制用户登录**：使用 `AllowUsers` 或 `AllowGroups` 指令只允许特定的用户或组进行 SSH 登录。
...
确保服务器上相应用户的 `~/.ssh` 目录和 `~/.ssh/authorized_keys` 文件的权限设置正确（`.ssh` 目录为 `700`，`authorized_keys` 文件为 `600`）。

### 配置未生效

每次修改 `/etc/ssh/sshd_config` 后，**必须重启 SSH 服务**。

### 防火墙阻挡

确保服务器防火墙放行了 SSH 所使用的端口（通常是 22 或你自定义的端口）。
...
在某些严格策略下，SELinux（Rocky Linux）或 AppArmor（Debian）可能会阻止 SSH 操作，必要时需调整策略或放行。
