在 Git 中，`--bare`、`--git-dir` 和 `--work-tree` 是三个控制仓库路径和工作目录行为的核心选项。理解它们对于搭建中央仓库、自定义运行环境或编写脚本非常有帮助。
...
| 选项          | 改变的对象                                                     | 默认行为                                 |
| ------------- | -------------------------------------------------------------- | ---------------------------------------- |
| `--git-dir`   | Git 数据库位置                                                 | 从当前目录向上寻找 `.git`                |
| `--work-tree` | 工作目录位置                                                   | 数据库所在目录的父目录（裸仓库无工作树） |
| `--bare`      | 同时设置：`--git-dir = 当前目录`<br>`--work-tree = 禁止工作树` | 无工作树，且不检查 `GIT_DIR` 环境变量    |
...
| 选项          | 一句话解释                              |
| ------------- | --------------------------------------- |
| `--bare`      | 创建无工作目录的仓库，仅作推送/拉取中心 |
| `--git-dir`   | 告诉 Git `.git` 数据库在哪里            |
| `--work-tree` | 告诉 Git 工作区文件在哪里               |
