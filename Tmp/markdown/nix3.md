欢迎来到 Nix 的新纪元。如果你之前被 Nix 复杂的语法或“老派”的安装方式劝退过，那么 **Nix Flakes** 和 **Nix-command**（即所谓的“新命令”）就是为你准备的。它们将 Nix 从一个充满魔法的黑盒变成了一个工程化、标准化的现代工具。

本教程将带你从核心概念直接跨越到生产力实践。

---

## 1. 核心思想：声明式与不可变性

在传统的 Linux 中，安装软件是“改变状态”（如往 `/usr/bin` 放文件）。在 Nix 中，安装软件是**计算结果**。
